...
func (p *ServiceNowPlugin) GrantAccess(ar *api.AccessRequest, app *argocd.Application) (*plugin.GrantResponse, error) {
	...
	grantedAccessTextUI := fmt.Sprintf("Granted access: change __%s__ (%s), until __%s (%s min)__", changeNumber, changeShortDescription, changeEndDate, string(changeRemainingTime))
